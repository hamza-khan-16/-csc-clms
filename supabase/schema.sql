-- ============================================================
-- CLMS — Chandrabhan Sharma College Leave Management System
-- Complete Supabase Schema  (run this on a fresh project)
-- ============================================================
...
CREATE TABLE public.lectures (
  ...
  lecture_date  date,                  -- set for one-off / substitution lectures
  created_at    timestamptz NOT NULL DEFAULT now()
);
...
CREATE TABLE public.holidays (
  ...
  department_id uuid REFERENCES public.departments(id) ON DELETE CASCADE, -- NULL = college-wide
  created_at    timestamptz NOT NULL DEFAULT now()
);
...
-- Count working days between two dates (excl. Sundays and holidays for a dept)
...
-- ── COLLEGE-ID ASSIGNMENT TRIGGER ───────────────────────────
-- Fires when a profile is approved; derives a college login ID from the name.
...
-- Called client-side via supabase.rpc('register_profile', {...})
...
CREATE POLICY "admin manages profiles"    ON public.profiles FOR ALL    TO authenticated
  USING (public.has_role(auth.uid(),'admin')) WITH CHECK (public.has_role(auth.uid(),'admin'));
CREATE POLICY "hod approves dept teachers" ON public.profiles FOR UPDATE TO authenticated
