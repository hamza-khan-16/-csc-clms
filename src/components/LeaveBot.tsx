...
export function LeaveBot() {
  ...
  const [messages, setMessages] = useState<Message[]>(() => {
    try { const s=sessionStorage.getItem(CHAT_KEY); if(s) return JSON.parse(s) as Message[]; } catch (e) { if (process.env.NODE_ENV==="development") console.warn("[LeaveBot chat cache read]",e); }
    return [INITIAL_MSG];
  });

  useEffect(() => {
    try { sessionStorage.setItem(CHAT_KEY, JSON.stringify(messages)); } catch (e) { if (process.env.NODE_ENV==="development") console.warn("[LeaveBot chat cache write]",e); }
  }, [CHAT_KEY, messages]);
        ...
        <>
          ...
          <div className="fixed bottom-36 right-2 left-2 z-50 sm:left-auto sm:right-4 sm:w-[420px] sm:bottom-20 flex flex-col shadow-2xl rounded-2xl border border-border bg-background overflow-hidden max-h-[70dvh] sm:max-h-[620px]"
            ...
            <div className="flex items-center gap-3 px-4 py-3 bg-gradient-to-r from-primary to-primary/80 text-primary-foreground flex-shrink-0">
              ...
              <Tooltip>
                <TooltipTrigger asChild>
                  <button onClick={()=>setOpen(false)} className="flex items-center justify-center size-8 rounded-full border-2 border-white text-white hover:bg-white hover:text-primary transition-colors" aria-label="Close">
                    <X className="w-4 h-4" />
                  </button>
                </TooltipTrigger>
