...
export function OnboardingTour({ role }: Props) {
  ...
  useEffect(() => {
    ...
    try {
      if (!localStorage.getItem(TOUR_KEY)) {
        // Slight delay so the dashboard fully loads first
  ...
  return (
    <>
      {/* Backdrop */}
      <div className="fixed inset-0 z-[300] bg-black/50 backdrop-blur-sm animate-in fade-in duration-300" />

      {/* Tour card */}
      <div className="fixed inset-x-4 bottom-24 z-[301] max-w-sm mx-auto animate-in slide-in-from-bottom-4 fade-in duration-300 sm:inset-auto sm:bottom-8 sm:right-8 sm:left-auto">
